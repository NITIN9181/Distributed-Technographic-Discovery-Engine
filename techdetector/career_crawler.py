...
def discover_career_pages(domain: str) -> CareerPageResult:
    ...
    if main_result.html:
        ...
        for a in soup.find_all("a", href=True):
            ...
            is_match = any(k in text for k in career_keywords) or any(
                k in href_str.lower() for k in ["career", "job"]
            )

            if is_match:
                full_url = urljoin(f"https://{domain}", href_str)
                # Ensure it's the same base domain
                if (
                    urlparse(full_url).netloc.endswith(domain)
