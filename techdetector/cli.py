...
from techdetector.config import load_config, Config
from techdetector.scanner import init_database, perform_scan
from techdetector.batch_scanner import scan_batch, BatchConfig, stream_domains, ScanProgress
...
def cli():
    ...
    logging.basicConfig(level=logging.ERROR) # Suppress normal logs for beautiful output
    load_config()
...
def scan(url: str, vectors: str):
    ...
    console.print(f"Scanned at: {result.scan_timestamp.strftime('%Y-%m-%d %H:%M:%S UTC')}")
    console.print("-" * 40)
    ...
    for d in result.detections:
        console.print(f"[green]OK[/] [bold]{d.technology.name:<25}[/] [{d.technology.category}] ({d.vector.value})")
...
def query(tech, vector):
    ...
    for r in res:
        ...
        table.add_row(r['canonical_domain'], r['technology_id'], r['detection_vector'], str(v))
...
def export(format, output, tech, vector, since):
    ...
    if vector: filters['vector'] = vector
...
def stats():
    ...
    for k, v in sorted(tech_counts.items(), key=lambda item: item[1], reverse=True)[:10]:
