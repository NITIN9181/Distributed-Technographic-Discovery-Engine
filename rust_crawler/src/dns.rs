...
impl DNSResolver {
    ...
    pub async fn resolve(&self, domain: &str) -> DNSRecords {
        ...
        match self.resolver.mx_lookup(domain).await {
            Ok(lookup) => records.mx_records = lookup.iter().map(|v| v.exchange().to_utf8()).collect(),
            Err(e) => tracing::debug!("Failed MX lookup for {}: {}", domain, e),
        }

        match self.resolver.txt_lookup(domain).await {
            Ok(lookup) => records.txt_records = lookup.iter().map(|v| String::from_utf8_lossy(&v.txt_data().concat()).into_owned()).collect(),
        ...
        match self.resolver.lookup(&cname_query, trust_dns_resolver::proto::rr::RecordType::CNAME).await {
            Ok(lookup) => {
