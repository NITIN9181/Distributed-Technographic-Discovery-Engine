...
pub async fn extract_tls_info(domain: &str) -> TLSInfo {
    ...
    let tcp_stream = match tokio::time::timeout(std::time::Duration::from_secs(5), TcpStream::connect(&addr)).await {
        Ok(Ok(stream)) => stream,
        Ok(Err(e)) => {
    ...
    let tls_stream: tokio_rustls::client::TlsStream<TcpStream> = match tokio::time::timeout(std::time::Duration::from_secs(5), connector.connect(server_name, tcp_stream)).await {
        Ok(Ok(stream)) => stream,
        Ok(Err(e)) => {
